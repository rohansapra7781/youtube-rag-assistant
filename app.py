...
@st.cache_resource(show_spinner="Fetching transcript with timestamps and indexing into FAISS...")
def setup_rag_chain(video_id: str, languages: list):
    ...
    # 1. Build Documents with Timestamp Metadata
    raw_docs = [
        Document(
...
# Session-based chat memory store
    ...
    if index_btn and yt_url:
        ...
        if not vid:
            ...
            if err:
                ...
                # Wrap with LCEL conversational memory
...
if "conversational_chain" not in st.session_state or st.session_state.conversational_chain is None:
    ...
    if user_question := st.chat_input("Ask anything about this video (follow-ups supported)..."):
